...
class MetaDocumentsProvider : DocumentsProvider() {
    ...
    override fun queryChildDocuments(
        ...
        } ?: throw FileNotFoundException("Failed to list children of $parentDocumentId")
        return cursor
    }
    ...
    override fun createDocument(
        ...
        if (dir != null) {
            val lastDot = if (mimeType != DocumentsContract.Document.MIME_TYPE_DIR) displayName.lastIndexOf('.') else -1
            val (nameBase, extSuffix) = if (lastDot > 0)
            ...
            while (file.existsWithoutFollowingLinks)
                file = File(dir, "$nameBase (${i++})$extSuffix")
            try {
    ...
    override fun deleteDocument(documentId: String) {
        ...
        if (!deleted) throw FileNotFoundException("Failed to delete document $documentId")
    }
    ...
    override fun moveDocument(
        ...
        if (sourceFile != null && targetDir != null) {
            ...
            if (!targetFile.existsWithoutFollowingLinks && sourceFile.renameTo(targetFile))
    ...
    override fun renameDocument(documentId: String, displayName: String): String {
        ...
        if (file != null) {
            ...
            if (parent != null) {
                ...
                if (!targetFile.existsWithoutFollowingLinks && file.renameTo(targetFile))
    ...
    override fun call(method: String, arg: String?, extras: Bundle?): Bundle? {
        enforceAccess()
        if (method == "android:moveDocument") {
            return synchronized(this) {
                val result = super.call(method, arg, extras)
                if (result?.getParcelable<Uri>("uri") != null) {
                    extras?.getParcelable<Uri>("uri")?.let {
                        revokeDocumentPermission(DocumentsContract.getDocumentId(it))
                    }
                }
                result
            }
        ...
        if (superResult != null || !method.startsWith("mt:") || extras == null)
            return superResult
        val documentId = extras.getParcelable<Uri>("uri")?.let {
    ...
    companion object {
        ...
        fun isEnabled(context: Context): Boolean {
            val component = ComponentName(context, MetaDocumentsProvider::class.java)
        ...
        private val File.mimeType: String
            ...
            else extension.lowercase().takeIf { it.isNotEmpty() }?.let {
